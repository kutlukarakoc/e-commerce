      ...
      return (
         <>
            {Array.from({ length: count }).map((item, index) => (
               <div key={index} className='bg-zinc-50 h-[525px] p-8 rounded-md relative animate-pulse'>
                  {hasIcon ? <div className='absolute top-2 right-3 w-5 h-5 rounded-full bg-gray-600'></div> : null}
