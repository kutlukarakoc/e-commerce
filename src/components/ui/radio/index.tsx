      ...
      <>
         {options.map((option: any) => (
            <div key={option.value} className='flex items-center gap-x-3'>
               <input data-cy='profile-gender' type='radio' id={option.value} name={name} className='peer sr-only' value={option.value} checked={selected === option.value} onChange={onChange} {...rest} />
