...
const Input: React.FC<InputProps> = ({ type, name, label, placeholder, wrapperStyles, inputStyles, ...rest }) => {
   return (
      <div className={'input-wrapper relative w-full ' + wrapperStyles}>
         ...
         <span className='absolute text-sm whitespace-nowrap left-0 translate-x-2 top-1/2 -translate-y-1/2 transition-all peer-focus-within:text-xs peer-focus:text-indigo-600 peer-focus:top-0 peer-focus:bg-white'>{placeholder}</span>
      </div>
   )
}
